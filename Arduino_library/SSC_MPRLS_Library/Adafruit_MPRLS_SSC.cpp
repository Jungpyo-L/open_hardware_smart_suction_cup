 ...
 * 
 * Changes by Jungpyo Lee (https://github.com/Jungpyo-L) for the Smart Suction Cup system
 */
...
float Adafruit_MPRLS::readPressure2(void) {
  uint32_t raw_psi = getData();
  if (raw_psi == 0xFFFFFFFF || _OUTPUT_min == _OUTPUT_max) {
    return NAN;
  }
...
/*!
    @Made by JP
    @brief Request measurement data from the device
*/
/**************************************************************************/
bool Adafruit_MPRLS::requestData(void) {
  uint8_t buffer[4] = {0xAA, 0, 0, 0};
...
*/
/**************************************************************************/
uint32_t Adafruit_MPRLS::getData(void) {
  uint8_t buffer[4] = {0xAA, 0, 0, 0};
